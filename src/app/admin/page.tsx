...
interface NewsItem { id: string; title: string; category: string; author: string; created_at: string; }
export default function AdminDashboard() {
  ...
  useEffect(() => { fetch("/api/news").then(r => r.json()).then(d => setNews(d || [])); }, []);
  async function addNews(e: React.FormEvent) {
    e.preventDefault();
    await fetch("/api/news", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, summary, content, category, image, author: "Admin" }) });
          ...
          <div className="lg:col-span-2 bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50"><tr><th className="text-left px-3 py-2">Başlık</th><th className="text-left px-3 py-2">Kategori</th><th className="text-left px-3 py-2">Tarih</th><th></th></tr></thead>
                <tbody>{news.map(n => (
