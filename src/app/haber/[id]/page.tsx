...
export async function getNews(id: string) { const { data } = await supabaseAdmin.from("news").select("*").eq("id", id).single(); return data; }
    ...
    <main className="min-h-screen bg-[#f1f1f1]">
      <header className="bg-white border-b border-gray-100">
        <div className="container mx-auto px-4 py-3">
          <Link href="/" className="inline-flex items-center gap-2 text-[#c41e3a] font-semibold text-sm hover:underline"><ArrowLeft size={16} /> Ana Sayfa</Link>
        </div>
      </header>
      <article className="container mx-auto px-4 py-6 max-w-3xl">
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 flex flex-wrap gap-2">
          {[news.category, "Şanlıurfa", "Son Dakika", "Urfa Haber"].map(t => <span key={t} className="px-2.5 py-1 bg-gray-100 text-gray-600 text-xs rounded">#{t}</span>)}
        </div>
      </article>
      {related.length > 0 && (
        <section className="bg-white border-t border-gray-100 py-8">
          <div className="container mx-auto px-4 max-w-3xl">
            <h2 className="text-base font-bold text-[#1a1a2e] mb-4 flex items-center gap-2"><span className="w-1 h-4 bg-[#c41e3a] rounded" /> İlgili Haberler</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {related.map(r => (
                <Link key={r.id} href={`/haber/${r.id}`} className="flex gap-3 items-start hover:bg-gray-50 p-2 rounded transition group">
                  <div className="relative w-16 h-16 shrink-0 rounded overflow-hidden"><Image src={r.image} alt={r.title} fill className="object-cover" /></div>
                  <div>
